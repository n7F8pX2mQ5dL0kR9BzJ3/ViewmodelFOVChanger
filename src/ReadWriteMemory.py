...
class Process(object):
    ...
    def get_modules(self) -> List[tuple]:
        """
        Get the process's modules along with their names.
        :return: A list of tuples, each containing a module's address (in decimal)
                    and its corresponding file path.
                    Returns an empty list if the process is not open.
        """
        ...
        for module in tuple(modules):
            if module:
                ...
                hmodule = ctypes.wintypes.HMODULE(module)  # Cast module to HMODULE
                ctypes.windll.psapi.GetModuleFileNameExW(self.handle, hmodule, buffer, MAX_PATH)
                module_info.append((module, buffer.value))
    ...
    def writeFloat(self, lp_base_address: int, value: float) -> bool:
        """
        Write a float value at the given memory address.
        :param lp_base_address: Memory address to write to.
        :param value: The float value to write.
        :return: True if the write succeeded; False otherwise.
        """
        # Pack the float into 4 bytes (little-endian)
        data_bytes = list(struct.unpack('B' * 4, struct.pack('<f', value)))
        return self.writeByte(lp_base_address, data_bytes)

    def readFloat(self, lp_base_address: int) -> float:
        """
        Read a float value from the given memory address.
        :param lp_base_address: Memory address to read from.
        :return: The float value read.
        """
        # Read 4 bytes from memory
        data_bytes = self.readByte(lp_base_address, 4)  
        byte_data = bytes(data_bytes)
        return struct.unpack('<f', byte_data)[0]

    def writeDouble(self, lp_base_address: int, value: float) -> bool:
        """
        Write a double (8-byte float) value at the given memory address.
        :param lp_base_address: Memory address to write to.
        :param value: The double value to write.
        :return: True if the write succeeded; False otherwise.
        """
        data_bytes = list(struct.unpack('B' * 8, struct.pack('<d', value)))
        return self.writeByte(lp_base_address, data_bytes)

    def readDouble(self, lp_base_address: int) -> float:
        """
        Read a double (8-byte float) value from the given memory address.
        :param lp_base_address: Memory address to read from.
        :return: The double value read.
        """
        data_bytes = self.readByte(lp_base_address, 8)
        return struct.unpack('<d', bytes(data_bytes))[0]
